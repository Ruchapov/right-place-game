...
export default function Explore({ onClose, endurance, strength, level, onRunComplete, mapFile: mapFileProp, token, trophies, armor, weaponDamage, equippedSkills }: ExploreProps) {
  ...
  if (setupError) {
    ...
    // Вторая строка зависит от причины: два самых частых отказа сервера — не
    // про связь, и совет "проверьте соединение" в них уводит не туда. Сверяем
    // по подстроке ответа сервера: setupError это err.message (см. вызов
    ...
    const hint =
      setupError.startsWith(SETUP_ERR_START_TIMEOUT)
        ...
        : setupError.includes('A run is already in progress')
          ...
          // появления подтверждения старта, закроется при входе как смерть —
          // с потерей банка и без возврата энергии.
          ? 'Прошлый забег ещё открыт на сервере. Закрой приложение полностью и открой снова: он закроется при входе.'
          : 'Проверьте соединение.'
    return (
      <StoneFrameScreen title="ЗАБЕГ ПРЕРВАН" lines={['Не удалось загрузить забег.', hint]}>
