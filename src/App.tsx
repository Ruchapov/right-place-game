...
export default function App() {
  ...
  async function handleBattleEnd(result: { won: boolean; damageTaken: number; damageDealt: number; skillUses: number; actualHpLost: number }) {
    setInBattle(false)
    const token = localStorage.getItem('jwt')
    if (!token) return
    setRunError(null)
    try {
      const br: BattleResult = await submitBattleResult(token, result.won, result.damageTaken, result.damageDealt, result.skillUses, result.actualHpLost)
  ...
  return (
    <div style={{ padding: 20, paddingBottom: 80, fontFamily: 'sans-serif', minHeight: '100vh', background: '#1a1a2e', color: 'white' }}>
      ...
      {inBattle && <Battle initialHp={runHp} maxHp={runMaxHp} isBoss={rooms ? rooms[roomIndex] === 'boss' : false} level={player?.level ?? 1} equippedSkills={player?.equippedSkills ?? []} potionCharges={player?.potionCharges ?? 0} onBattleEnd={handleBattleEnd} />}
