...
// --- Верх ТЕЛА для проверки шипов (isTouchingSpikes) ---
// ТОЛЬКО для шипов. К физике, боевому хитбоксу и JUMP_HIT_OFFSET_Y отношения
// НЕ имеют: тот подобран тюнером под ощущение прыжка, а эти два числа
// ЗАМЕРЕНЫ по альфа-каналу спрайта и означают, где у героя реально макушка
// относительно phys.y. Разница принципиальна: phys.y — верх грубого бокса
// (PLAYER_HEIGHT=128), и он выше настоящей головы, из-за чего шип на узкой
// платформе '=' бил по воздуху над героем ещё до того, как тот упирался в
// платформу.
//
// Два числа, а не одно, потому что верх тела реально ездит по позе на ~44px:
// у кадров прыжка ноги поджаты, и якорь смещён (RISE_ANCHOR_Y/FALL_ANCHOR_Y
// ниже), поэтому в полёте вся фигура рисуется ниже. Выбор — по phys.onGround.
// Внутри каждой группы взят МИНИМУМ (самая высокая макушка), чтобы проверка
// не пропустила реальную голову ни в одной позе группы.
//
// КАК ЗАМЕРЕНО (повторить при замене анимации героя — иначе числа молча
// разъедутся с новым спрайтом):
//   1) topRow — первая сверху строка клетки спрайт-листа, где alpha > 8;
//   2) в воздухе hero.y = phys.y + PLAYER_HEIGHT + FOOT_TUNE
//      (Explore.tsx: findGroundSurfaceY возвращает null, остаётся низ бокса);
//   3) offset = PLAYER_HEIGHT + FOOT_TUNE
//               + (topRow - anchorY * HERO_CELL_H) * (HERO_DRAW_H / HERO_CELL_H)
//      при HERO_DRAW_H/HERO_CELL_H = 140/296 ≈ 0.472973.
// Замеры на текущем наборе спрайтов:
//   земля:  idle.png, все 24 кадра, anchorY = GROUND_ANCHOR_Y = 1.0,
//           topRow = 10            -> 3.73  <- МИНИМУМ группы, взят
//           run.png, 21 кадр,      anchorY = 1.0,
//           topRow = 32..42        -> 14.14..18.86
//   воздух: jump.png кадр RISE_FRAME = 9,  anchorY = RISE_ANCHOR_Y = 0.729,
//           topRow = 22            -> 47.35
//           jump.png кадр FALL_FRAME = 14, anchorY = FALL_ANCHOR_Y = 0.816,
//           topRow = 15            -> 31.85 <- МИНИМУМ группы, взят
// В воздухе показываются ТОЛЬКО кадры 9 и 14 (см. тикер Explore.tsx), поэтому
// воздушная группа состоит из них двоих, а не из всего листа jump.png.
export const SPIKE_BODY_TOP_GROUND = 3.73
export const SPIKE_BODY_TOP_AIR = 31.85
...
// Каст (поднятие руки → удержание → опускание) — анимация под скилл fireball.
// Лист ПЕРЕРИСОВАН — в работе ВТОРАЯ версия (cast_v2), первая удалена из
// репозитория и в коде не упоминается больше нигде: сетка
// та же, что у остальных листов героя и что у прошлой версии — 24 кадра,
// 12 колонок, 2 ряда, клетка 394×296, лист 4728×592, без лупа. cols НЕ
// передаётся явно — дефолт loadSheetFrames равен 12 и здесь верен.
// ⚠️ Смена листа тянет за собой ДВА числа ниже: CAST_SPAWN_FRAME (кадр
// вылета) и FIREBALL_OFFSET_X/Y (ладонь на этом кадре) — они замерены по
// ЭТОМУ листу, при следующей перерисовке пересчитывать оба.
export const HERO_CAST_SRC = `${import.meta.env.BASE_URL}assets/sprites/hero/cast_v2.png`
export const HERO_CAST_COUNT = 24
export const HERO_CAST_ROWS = 2
...
// вниз (гейт `stage === 1` в damageBoss, Explore.tsx), не каждый кадр. Гейт
// живёт в ОБЩЕЙ точке урона, поэтому стадию переключает любой источник —
// меч, кровотечение slash, взрыв fireball, рывок dash (раньше он стоял в
// applyAttackHit и срабатывал только на удар мечом).
// Что даёт стадия 2: Melee2 (50/50 с обычным melee) и топот — оба решения
// принимаются в AI, см. boss.stage в entities/boss.ts. Записи "только
// console.log" в этом комментарии больше не соответствовали коду.
...
// Память о цели у босса — тот же смысл и то же значение, что AGGRO_MEMORY_MS
// у зверя (см. там же про прыжок и про строгий sameFloor), но СВОЯ
// константа: радиус агро у босса тоже свой (20 тайлов против 7), и сводить
// их в одно число значило бы связать два независимых баланса.
export const BOSS_AGGRO_MEMORY_MS = 2000
...
// Размер отрисовки и скорость анимации заданы ниже в этом же блоке
// (SLASH_STREAK_DRAW_W/H, SLASH_STREAK_ANIM_SPEED) — подобраны тюнером вживую.
...
// Скилл slash (числа восстановлены из удалённого Battle.tsx, см. CLAUDE.md,
// раздел Skills, блок "Slash + кровотечение"). Кулдаун в МИЛЛИСЕКУНДАХ, как
// HEAL_COOLDOWN_MS и PLAYER_DODGE_COOLDOWN_MS.
export const SLASH_COOLDOWN_MS = 10000
// Дальность slash = PLAYER_ATTACK_RANGE × этот множитель. Хитбокс строится по
// тому же правилу, что у обычной атаки (прямоугольник перед игроком по
// facing), просто шире.
export const SLASH_RANGE_MULT = 1.5
// Кровотечение: 3% от МАКСИМАЛЬНОГО hp цели за тик, тик раз в секунду,
// 5 секунд — до 5 тиков, суммарно до 15% макс. hp. Прямого урона у самого
// skill НЕТ, весь урон скилла идёт отсюда.
export const BLEED_FRAC_PER_TICK = 0.03
export const BLEED_TICK_MS = 1000
export const BLEED_DURATION_MS = 5000

// VFX дуги взмаха. Размер отрисовки — НЕ размер кадра (160×174): Pixi рисует
// текстуру в любом размере. Подобрано вживую тюнером.
// ⚠️ Аспект 284/176 = 1.61 против 0.92 у кадра — дуга РАСТЯНУТА поперёк почти
// вдвое относительно исходной пропорции. Это осознанный подбор (широкий
// росчерк перед героем), а не ошибка нарезки листа.
// Смещение — от центра героя, по facing (X зеркалится, Y нет).
export const SLASH_STREAK_DRAW_W = 284
export const SLASH_STREAK_DRAW_H = 176
export const SLASH_STREAK_OFFSET_X = 20
export const SLASH_STREAK_OFFSET_Y = -4
export const SLASH_STREAK_ANIM_SPEED = 0.5

// Скилл dash — рывок вперёд по facing: своя анимация героя (подмена текстур
// на его спрайте, не VFX-оверлей), смещение по горизонтали, урон целям, через
// которые прошёл, неуязвимость на время рывка и кулдаун.
//
// Лист 2640×128 (сверен замером альфы, а не по докам: запись "8200×128" в
// docs/entity-modules.md устарела). ОДНОРЯДНЫЙ: высота листа равна высоте
// кадра, 2640/11 = 240 без остатка. COLS=11 передаётся ЯВНО — дефолт
// loadSheetFrames равен 12, и на нём нарезка молча съехала бы.
export const DASH_SRC = `${import.meta.env.BASE_URL}assets/skills/Dash_Strike.png`
export const DASH_CELL_W = 240
export const DASH_CELL_H = 128
export const DASH_COUNT = 11
export const DASH_COLS = 11

// Отрисовка dash. Это анимация САМОГО ГЕРОЯ (подмена текстур на его спрайте),
// а не VFX-оверлей поверх — но клетка у листа своя (240×128 против 394×296 у
// остальных листов героя), поэтому высота отрисовки и смещение тоже свои:
// общий множитель героя (HERO_DRAW_H / 296) дал бы здесь ~61px вместо ~140.
// DASH_OFFSET_X зеркалится по facing, DASH_OFFSET_Y — нет (тот же приём, что
// у SLASH_STREAK_OFFSET_*).
// Значения подобраны вживую отладочной панелью (удалена по окончании работы
// над dash). Смещения по итогам подбора оказались нулевыми: герой на листе
// dash стоит там же, где нужно — поэтому обе константы 0, а не потому, что
// до них не дошли руки.
export const DASH_DRAW_H = 116
export const DASH_OFFSET_X = 0
export const DASH_OFFSET_Y = 0
// 11 кадров / (60fps × 0.33) ≈ 0.51с — тот же порядок, что у остальных разовых
// анимаций героя: атака 14 кадров при ATTACK_ANIM_SPEED=0.5 ≈ 0.47с, hurt
// 10 при 0.45 ≈ 0.37с.
export const DASH_ANIM_SPEED = 0.33
// Длина рывка в пикселях мира. 256 = 4 тайла по 64. Для масштаба: анимация
// доигрывает до последнего кадра за (11-1)/0.33 ≈ 30 кадров тикера ≈ 0.5с, а
// шагом (MOVE_SPEED=4 px/кадр) герой за это же время прошёл бы ≈121px — то
// есть рывок примерно вдвое быстрее ходьбы.
// ⚠️ Скорость рывка ОТДЕЛЬНОЙ константой не заводится намеренно: она выведена
// из этой дистанции и длительности анимации (см. ветку dash в тикере
// Explore.tsx), иначе рывок и его анимация разъезжались бы при правке любого
// из двух чисел.
export const DASH_DISTANCE = 256
// Кулдаун рывка в МИЛЛИСЕКУНДАХ, как HEAL_COOLDOWN_MS и SLASH_COOLDOWN_MS.
// 5с — по docs/skills.md ("heal/fireball/iceball/dash — 5с, slash — 10с");
// у slash 10с потому, что он единственный с таким кулдауном, не наоборот.
export const DASH_COOLDOWN_MS = 5000
// Урон рывка: доля от МАКСИМАЛЬНОГО hp цели за одно попадание — тот же
// способ, что у BLEED_FRAC_PER_TICK выше (единственный уже существующий в
// коде подход к урону скилла), чтобы не заводить третий.
// Значение ВРЕМЕННОЕ: общей формулы урона скиллов в проекте пока нет, она
// придёт отдельной задачей сразу на все скиллы и заменит это число вместе с
// HEAL_PULSE_FRAC и BLEED_FRAC_PER_TICK.
export const DASH_DAMAGE_FRAC = 0.06

// Скилл fireball — ЭТАП 2: полёт, импакт, попадание по цели, урон и кулдаун
// (FIREBALL_DAMAGE_FRAC/FIREBALL_COOLDOWN_MS в конце блока). Iceball, у
// которого те же два листа своего цвета, этим этапом НЕ делался.
//
// Снаряд: лист 1600×100 (замер), ОДНОРЯДНЫЙ, 1600/10 = 160 без остатка.
// Импакт: лист 1920×145 (замер), ОДНОРЯДНЫЙ, 1920/12 = 160 без остатка.
// COLS у обоих передаётся ЯВНО — дефолт loadSheetFrames равен 12, и на
// снаряде (10 колонок) нарезка молча съехала бы.
export const FIREBALL_PROJECTILE_SRC = `${import.meta.env.BASE_URL}assets/skills/Fireball_Projectile.png`
export const FIREBALL_PROJECTILE_CELL_W = 160
export const FIREBALL_PROJECTILE_CELL_H = 100
export const FIREBALL_PROJECTILE_COUNT = 10
export const FIREBALL_PROJECTILE_COLS = 10
export const FIREBALL_IMPACT_SRC = `${import.meta.env.BASE_URL}assets/skills/Fireball_Impact.png`
export const FIREBALL_IMPACT_CELL_W = 160
export const FIREBALL_IMPACT_CELL_H = 145
export const FIREBALL_IMPACT_COUNT = 12
export const FIREBALL_IMPACT_COLS = 12

// Отрисовка. Задаётся ВЫСОТА, ширина считается из пропорции клетки — тот же
// приём, что у импакта шипа босса (BOSS_SPIKE_IMPACT_DRAW_H): так подбирать
// надо одно число, а не два, и картинка не растягивается.
// Числа ПОДОБРАНЫ вживую и запечены: отладочная панель, которой их искали,
// удалена, код читает эти константы напрямую. Общие для fireball и iceball.
export const FIREBALL_DRAW_H = 46
export const FIREBALL_IMPACT_DRAW_H = 110
// Точка вылета — от ЦЕНТРА хитбокса героя (тот же приём, что у
// SLASH_STREAK_OFFSET_*): X зеркалится по facing, Y нет.
//
// Числа посчитаны от ЛАДОНИ на кадре вылета (CAST_SPAWN_FRAME), а не взяты
// на глаз. Пересчитаны под НОВЫЙ лист cast_v2 (на ПРЕДЫДУЩЕЙ версии листа
// ладонь была в другом месте клетки, старые +31/−45 к этому листу отношения
// не имеют). Центр ладони внутри клетки — (309, 113) (замер листа cast_v2),
// якорь спрайта героя — (0.5, 1.0), то есть точка (197, 296) той же клетки.
// Масштаб отрисовки = HERO_DRAW_H / HERO_CELL_H = 140/296 ≈ 0.473.
//   X: (309 − 197) × 0.473 = 112 × 0.473 ≈ +53.0, а спрайт героя стоит ровно
//      над центром хитбокса (hero.x = phys.x + PLAYER_WIDTH/2) — та же база,
//      что у снаряда, значит смещение переносится как есть.
//   Y: (113 − 296) × 0.473 = −183 × 0.473 ≈ −86.6 — но ОТ hero.y, а тот стоит
//      не на phys.y + PLAYER_HEIGHT, а на найденной поверхности пола плюс
//      FOOT_TUNE. На ровном полу hero.y ≈ phys.y + PLAYER_HEIGHT + FOOT_TUNE,
//      а база снаряда — phys.y + PLAYER_HEIGHT/2, отсюда
//      PLAYER_HEIGHT/2 + FOOT_TUNE − 86.6 = 64 + 11 − 86.6 ≈ −11.6.
// ⚠️ Обе базы совпадают ТОЛЬКО на ровном полу: hero.y привязан к поверхности
// тайла, а phys.y — к хитбоксу, и на склоне/в воздухе они расходятся. Каст
// прерывается прыжком, так что случай «в воздухе» до вылета не доживает.
// Расчёт ПОДТВЕРЖДЁН на экране (панель подбора удалена, числа запечены):
// шар выходит из ладони.
export const FIREBALL_OFFSET_X = 53
export const FIREBALL_OFFSET_Y = -12
// Скорость в px/СЕКУНДУ (как BOSS_SPIKE_SPEED_X, а не px/кадр как MOVE_SPEED)
// — снаряд шагает по реальным секундам, см. updateProjectiles в skills.ts.
// ⚠️ ПОТОЛОК СКОРОСТИ. Проверка ГЕОМЕТРИИ у снаряда ТОЧЕЧНАЯ (одна точка —
// центр спрайта), свипа между кадрами нет — как и у шипа босса (проверка по
// ЦЕЛЯМ, добавленная этапом 2, идёт прямоугольником спрайта и здесь ни при
// чём: она шире и промахивается позже). Значит шаг за
// кадр обязан оставаться заметно меньше тайла (TILE_SIZE=64), иначе снаряд
// проскочит сквозь стену. При 700 px/с шаг ≈12px при 60fps и ≈23px при 30fps
// — запас больше двух раз. Верхняя граница ползунка в тюнере выставлена
// 1600 px/с ИМЕННО поэтому: там шаг ≈27px при 60fps и ≈53px при 30fps, ещё
// под тайлом. Поднимать выше без свипа нельзя.
export const FIREBALL_SPEED = 700
export const FIREBALL_LIFETIME_MS = 2500
// Снаряд ЛУПИТСЯ (горящий шар в полёте), импакт — разовый.
export const FIREBALL_ANIM_SPEED = 0.4
export const FIREBALL_IMPACT_ANIM_SPEED = 0.35
// Урон снаряда: доля от МАКСИМАЛЬНОГО hp цели за одно попадание — тот же
// способ, что у DASH_DAMAGE_FRAC и BLEED_FRAC_PER_TICK выше, чтобы не
// заводить третий подход к урону скиллов.
// Значение ВРЕМЕННОЕ: общей формулы урона скиллов в проекте пока нет, она
// придёт отдельной задачей сразу на все скиллы и заменит это число вместе с
// HEAL_PULSE_FRAC, BLEED_FRAC_PER_TICK и DASH_DAMAGE_FRAC.
// Больше, чем у рывка (0.06): снаряд бьёт ОДНУ цель и гаснет на ней, а рывок
// проходит сквозь строй и собирает всех, через кого проехал.
export const FIREBALL_DAMAGE_FRAC = 0.1
// Глубина захода в цель. Взрыв запускается НЕ от касания углов боксов, а
// только когда бокс снаряда и бокс цели перекрылись минимум на столько px
// ПО ОБЕИМ осям: иначе снаряд гас, визуально не долетев до тела.
// ⚠️ ЭТО ПОРОГ, А НЕ ЗАПАС: он не может быть больше максимально возможного
// перекрытия с целью, иначе цель перестаёт детонировать снаряд ВООБЩЕ. Два
// узких места на сегодняшних числах:
//   — по вертикали со зверем: снаряд летит на 76px над полом при высоте
//     бокса 46, бокс зверя — 80px от ног, полоса перекрытия на ровном полу
//     всего ≈27px (53..80 над полом);
//   — по горизонтали с боссом: его бокс шириной 60px (BOSS_WIDTH).
// Отсюда рабочий потолок ≈27 — и подбор на экране остановился ровно на нём:
// снаряд входит в тело настолько глубоко, насколько это вообще возможно, не
// теряя попаданий. Значение ОБЩЕЕ для fireball и iceball.
// ⚠️ Больше ставить нельзя: 28 и выше — и зверь на ровном полу перестаёт
// детонировать снаряд ВООБЩЕ (перекрытия по вертикали столько уже нет).
export const FIREBALL_HIT_OVERLAP = 27
// Зона урона взрыва — прямоугольник вокруг точки смерти снаряда, из ТЕХ ЖЕ
// чисел, что и спрайт импакта (высота FIREBALL_IMPACT_DRAW_H, ширина по
// пропорции клетки FIREBALL_IMPACT_CELL_W/H, центр в точке попадания).
// Множитель разводит зону поражения с картинкой, не трогая визуал: 1.0 —
// зона ровно по вспышке (121.4×110 при IMPACT_DRAW_H=110), больше 1 — бьёт
// шире, чем видно, меньше 1 — уже.
// ⚠️ Разведение зоны и картинки — это осознанная ложь игроку про радиус.
// 1.0 стоит именно поэтому: пока не доказано обратное живой игрой, зона
// поражения должна совпадать с тем, что игрок видит.
export const FIREBALL_BLAST_SCALE = 1.0
// Кулдаун в МИЛЛИСЕКУНДАХ, как HEAL_COOLDOWN_MS/SLASH_COOLDOWN_MS/
// DASH_COOLDOWN_MS. 5с — по docs/skills.md ("heal/fireball/iceball/dash —
// 5с, slash — 10с").
// ⚠️ Тратится НЕ на нажатии, а на кадре вылета (см. onCastSpawnFrame в
// skills.ts): каст, оборванный до этого кадра прыжком/хитстаном/смертью,
// кулдаун не съедает — снаряда в этом случае не было вообще.
export const FIREBALL_COOLDOWN_MS = 5000

// Скилл iceball — ТОТ ЖЕ снаряд, что fireball, на своих текстурах. Общий код
// (полёт, геометрия, глубина захода, взрыв по площади, impact, порядок
// причин смерти) живёт в explore/entities/skills.ts один раз на оба скилла и
// параметризуется спекой — см. ProjectileSpec там же.
// Отсюда и состав этого блока: СВОИ у iceball только листы, урон, кулдаун и
// эффект попадания. Всё остальное (скорость, время жизни, размеры отрисовки,
// смещение вылета, FIREBALL_HIT_OVERLAP, FIREBALL_BLAST_SCALE, скорости
// анимаций) он берёт из FIREBALL_* выше — это общие «снарядные» числа, а не
// «числа огня»: подбор на экране показал, что разводить их незачем, оба
// снаряда летят и рвут одинаково. Понадобится развести — достаточно добавить
// своё поле в ProjectileSpec (skills.ts), fireball это не заденет.
//
// Листы сверены замером: 1600×100 (10 кадров) и 1920×145 (12 кадров) — до
// пикселя те же, что у fireball, но константы СВОИ: совпадение размеров это
// свойство текущего арта, а не правило, и общая константа связала бы два
// набора файлов в один.
export const ICEBALL_PROJECTILE_SRC = `${import.meta.env.BASE_URL}assets/skills/IceBall_Projectile.png`
export const ICEBALL_PROJECTILE_CELL_W = 160
export const ICEBALL_PROJECTILE_CELL_H = 100
export const ICEBALL_PROJECTILE_COUNT = 10
export const ICEBALL_PROJECTILE_COLS = 10
export const ICEBALL_IMPACT_SRC = `${import.meta.env.BASE_URL}assets/skills/IceBall_Impact.png`
export const ICEBALL_IMPACT_CELL_W = 160
export const ICEBALL_IMPACT_CELL_H = 145
export const ICEBALL_IMPACT_COUNT = 12
export const ICEBALL_IMPACT_COLS = 12
// Урон — тем же способом, что у FIREBALL_DAMAGE_FRAC/DASH_DAMAGE_FRAC/
// BLEED_FRAC_PER_TICK: доля МАКСИМАЛЬНОГО hp цели, чтобы не заводить
// четвёртый подход к урону скиллов.
// Значение ВРЕМЕННОЕ: общей формулы урона скиллов в проекте пока нет, она
// придёт отдельной задачей сразу на все скиллы и заменит это число вместе с
// HEAL_PULSE_FRAC, BLEED_FRAC_PER_TICK, DASH_DAMAGE_FRAC и
// FIREBALL_DAMAGE_FRAC.
// Меньше, чем у fireball (0.10): iceball платит уроном за контроль — стан на
// ICEBALL_STUN_MS ниже.
export const ICEBALL_DAMAGE_FRAC = 0.04
// Кулдаун — в МИЛЛИСЕКУНДАХ и тем же путём, что FIREBALL_COOLDOWN_MS:
// взводится НЕ на нажатии, а на кадре вылета (см. onCastSpawnFrame в
// skills.ts), поэтому оборванный каст его не съедает.
export const ICEBALL_COOLDOWN_MS = 5000
// Стан целей в зоне взрыва — ПАУЗА, а не отмена: цель замирает в том кадре,
// в котором её застало, и по окончании продолжает прерванное действие с того
// же места (замах доигрывается и бьёт — даже если игрок успел отойти).
// Реализация — заслон в начале обновления сущности (см. stunTimer в
// entities/enemy.ts и entities/boss.ts), поэтому на паузу встают И AI, И
...
// Каст — тем же приёмом, что атака: снаряд рождается на КАДРЕ анимации, не в
// момент нажатия (см. ветку cast в тикере и onCastSpawnFrame в skills.ts).
// Индекс 0-based. 11 — кадр, на котором рука вытянута полностью (замер листа
// cast_v2.png). Позу держат кадры 10–14, так что соседний индекс из этого
// окна тоже рабочий — есть чем двигать при подборе ползунком CAST_FRAME.
export const CAST_SPAWN_FRAME = 11
// 24 кадра / (60fps × 0.5) = 0.8с на всю анимацию, вылет на 11-м кадре —
// через 0.37с после нажатия. Тот же порядок, что у атаки (14 кадров при
// ATTACK_ANIM_SPEED=0.5 ≈ 0.47с).
export const CAST_ANIM_SPEED = 0.5
...
// (замирал) прямо во время погони.
// ⚠️ 2.5 тайла = 160px, а высоту прыжка это БОЛЬШЕ НЕ ПЕРЕКРЫВАЕТ (прежняя
// запись здесь утверждала обратное и устарела): при JUMP_VELOCITY=10 апекс
// был ≈161px и впритык укладывался, после подъёма до 11.5 стал
// v²/(2·GRAVITY) = 11.5²/0.62 ≈ 213px. Ноги героя проводят выше порога
// примерно половину полёта.
// Разрыв закрывается НЕ порогом, а ПАМЯТЬЮ агро (AGGRO_MEMORY_MS ниже и
// BOSS_AGGRO_MEMORY_MS): условие sameFloor осталось строгим — иначе на
// многоэтажных картах враг агрился бы сквозь пол, — а прыжок переживается
// тем, что цель помнят ещё несколько секунд после потери условия.
// Поднимать это число под высоту прыжка НЕЛЬЗЯ по той же причине.
export const SAME_FLOOR_TOLERANCE_TILES = 2.5

// Память о цели (зверь). Пока условие агро выполняется — таймер каждый кадр
// взводится в это значение; как только перестало (прыжок, уход за радиус,
// заход на другой этаж) — тикает вниз, и всё это время враг ведёт себя как
// агрённый. Ноль — обычная потеря цели, поведение прежнее (уход в патруль).
// 2с покрывают полный прыжок героя (≈1.24с полёта) с запасом больше чем
// вдвое и при этом не превращают зверя в вечного преследователя: убежав за
// радиус, игрок отрывается через те же 2с.
export const AGGRO_MEMORY_MS = 2000

// Расследование — реакция на урон ИЗДАЛЕКА (см. задачу): получив урон вне
// агро (снаряд/кровотечение с дистанции), враг идёт к точке, откуда прилетело,
// вместо того чтобы продолжать патруль как ни в чём не бывало.
// INVESTIGATE_MS — потолок ХОДЬБЫ к точке. Заканчивается раньше, если враг
// дошёл, упёрся в стену или в край платформы (см. enemy.ts/boss.ts).
// INVESTIGATE_LOOK_MS — пауза ПОСЛЕ ходьбы: стоит на месте лицом к точке,
// потом возвращается к обычному поведению (зверь — патруль, босс — стоит).
// Обе — общие для зверя и босса: это одна и та же реакция "услышал удар",
// разводить её по двум наборам чисел незачем (в отличие от радиусов агро,
// где у босса свой масштаб).
// ⚠️ Обычное агро ГЛАВНЕЕ: как только выполняется условие агро или жива его
// память, расследование сбрасывается и враг переходит в погоню.
export const INVESTIGATE_MS = 5000
export const INVESTIGATE_LOOK_MS = 2000
...
// Два окна прощения прыжка. Оба тикают по ticker.deltaMS (реальное время), как
// spikeIframe/dodgeCooldown/hurtTimer — иначе на просевшем FPS окна стали бы
// короче в реальных секундах, то есть игра «ужесточалась» бы ровно там, где
// игроку и так тяжело.
//
// JUMP_BUFFER_MS — нажатие, сделанное ДО опоры, ждёт её столько. Раньше флаг
// гасился безусловно, и нажатие за миг до приземления исчезало молча.
//
// COYOTE_MS — сколько ещё можно прыгнуть ПОСЛЕ схода с края. 80 мс при
// MOVE_SPEED=4 (240 px/с) это 19 px = 0.3 тайла: меньше гранулярности, которой
// размечены карты (целые тайлы), так что ни один проём не меняет категорию —
// мягче становится только тайминг на прыжках, уже стоявших впритык.
// Буфер намеренно щедрее койота: ложное срабатывание буфера стоит кадра
// задержки, ложный койот — прыжка из воздуха.
