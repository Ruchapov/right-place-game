...
export type Enemy = {
  ...
  // Память о цели (см. AGGRO_MEMORY_MS): мс, пока >0 враг ведёт себя как
  // агрённый, даже если условие агро прямо сейчас не выполняется. Взводится
  // в полное значение каждый кадр, пока условие выполняется, иначе тикает
  // вниз. Декремент — БЕЗУСЛОВНЫЙ, до гейтов hurtTimer/windingUp (иначе
  // память не истекала бы в хитстане и в замахе, см. enemy.ts).
  aggroMemoryTimer: number
  // Расследование (см. INVESTIGATE_MS/INVESTIGATE_LOOK_MS): X источника
  // последнего урона в МИРОВЫХ координатах и два таймера — ходьбы к точке и
  // паузы "осмотреться" после неё.
  // investigateX — null, когда расследовать нечего; НЕ ноль-заглушка: ноль это
  // валидная координата (левый край карты), и по нему нельзя было бы отличить
  // "точка в начале карты" от "точки нет". Читается только вместе с
  // ненулевым таймером.
  // Ставятся в damageEnemy/damageBoss (Explore.tsx) — единственной общей точке
  // списания HP, поэтому реагируют ВСЕ источники: меч, кровотечение, взрыв
  // fireball, рывок dash.
  // Декремент — БЕЗУСЛОВНЫЙ, до гейтов AI, как у aggroMemoryTimer.
  investigateX: number | null
  investigateTimer: number
  investigateLookTimer: number
  // Стан от iceball (см. ICEBALL_STUN_MS): мс, тикает вниз БЕЗУСЛОВНО, до
  // всех гейтов. Пока >0 — заслон в начале обновления сущности: AI, движение
  // и продвижение кадров анимации не выполняются, спрайт стоит на том кадре,
  // на котором его застало. Это ПАУЗА, а не отмена — прерванный замах после
  // стана доигрывается и бьёт, даже если игрок успел отойти (осознанно).
  stunTimer: number
...
// ВНИЗ (не каждый кадр, см. гейт `stage === 1` в damageBoss, Explore.tsx) —
// от ЛЮБОГО источника урона, включая скиллы, а не только от удара мечом.
...
export type Boss = {
  ...
  stunCount: number
  poiseImmuneTimer: number
  // Память о цели (см. BOSS_AGGRO_MEMORY_MS) — тот же смысл и то же
  // устройство, что aggroMemoryTimer у Enemy: пока >0, босс ведёт себя как
  // агрённый. Декремент БЕЗУСЛОВНЫЙ, до гейта hurtTimer/анимаций атак — иначе
  // память стояла бы ровно тогда, когда должна истекать (см. boss.ts).
  aggroMemoryTimer: number
  // Расследование (см. INVESTIGATE_MS/INVESTIGATE_LOOK_MS): X источника
  // последнего урона в МИРОВЫХ координатах и два таймера — ходьбы к точке и
  // паузы "осмотреться" после неё.
