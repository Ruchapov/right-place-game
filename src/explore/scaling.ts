...
// Урон — Math.ceil, не round (см. задачу "HP должно быть всегда целым"):
// округление не должно снижать реальный урон в пользу игрока.
...
// Все четыре — урон, тот же Math.ceil и по той же причине, что у
// scaledEnemyAttackDamage выше.
export function scaledBossMeleeDamage(level: number): number {
  return Math.ceil(C.BOSS_MELEE_DMG_MULT * scaledEnemyAttackDamage(level))
}

export function scaledBossMelee2Damage(level: number): number {
  return Math.ceil(C.BOSS_MELEE2_DMG_MULT * scaledEnemyAttackDamage(level))
}

export function scaledBossSpikeDamage(level: number): number {
  return Math.ceil(C.BOSS_SPIKE_DMG_MULT * scaledEnemyAttackDamage(level))
}

export function scaledBossWaveDamage(level: number): number {
  return Math.ceil(C.BOSS_WAVE_DMG_MULT * scaledEnemyAttackDamage(level))
}
